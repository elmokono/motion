...
def procesar_video(video_path, json_path):
    ...
    try:
        ...
        REGLA PARA BEBES:
        - 'hay_bebes' debe ser true solo si aparece un bebe o un nino muy pequeno
          (aproximadamente menor de 3 anos): en brazos, en cochecito o carrito, gateando,
          o dando sus primeros pasos.
        - Debe ser false para ninos que ya caminan y corren con soltura, adolescentes o
          adultos. Ante la duda, false.
        - Un bebe cuenta ademas como persona, asi que 'hay_personas' tambien debe ser true.
        REGLA PARA VISION NOCTURNA:
        - 'vision_nocturna' debe ser true si la camara esta en modo infrarrojo/nocturno,
          es decir si la imagen se ve en blanco y negro o escala de grises, o si la escena
          es claramente de noche (oscuridad, iluminacion artificial puntual).
        - Debe ser false si la imagen tiene colores naturales de luz diurna.
        """
        ...
        # A veces el modelo devuelve [{...}] en vez de {...}. Sin normalizar, el JSON
        # queda como lista y la UI no encuentra ninguna clave: el evento aparece como
        # "Otros" y nunca puede marcarse sospechoso. Habia 6 casos asi al 2026-09-17.
        if isinstance(metadata, list):
            metadata = metadata[0] if metadata and isinstance(metadata[0], dict) else {}
        if not isinstance(metadata, dict):
            print(f"Respuesta inesperada para {video_path}, se descarta: {type(metadata).__name__}")
            return
